...
const TeamSquadComponents = ({ id }: MatchPageProps) => {
  ...
  const handleSubmitAllSelections = async () => {
    ...
    try {
      ...
      console.log('This is the match state data')
  ...
  const isHighlighted = (playerId: number) =>
    ...
    return (
      <div className="my-8 px-6">
        ...
        {/* Checkbox for selecting innings */}
                    ...
                    {isInningSelected === 'second' && (
                      <>
                        <td className="border-b p-3">
                          <button
                            title="Select Batter 1"
                            className={`px-2 py-1 rounded ${player.playerId !== undefined && isSelected(player.playerId, 'batter1Id') ? 'bg-red-500 text-white' : 'text-red-500 hover:text-red-700'}`}
                            onClick={() => player.playerId !== undefined && handlePlayerSelection(player.playerId, 'batter1Id')}
                          >
                            Batter 1
                          </button>
