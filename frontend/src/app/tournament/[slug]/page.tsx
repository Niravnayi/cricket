  ...
  const renderMatches = (matches: Match[]) => (
    <ul className="space-y-6 mt-4">
      ...
      <Link href={`/matches/${match.matchId}`} className='space-y-14'>
        <li
          key={match.matchId}
          className="p-6 border rounded-lg bg-gray-100 shadow-md group transform transition-all hover:shadow-xl hover:-translate-y-2"
        >
          ...
          <p className="text-gray-600">Date: {new Date(match.dateTime).toLocaleString()}</p>
          ...
          {match.scorecard && (
            <div className="mt-4 text-gray-600">
              <p>
                Score: {match.scorecard.teamAScore}/{match.scorecard.teamAWickets} vs{' '}
                {match.scorecard.teamBScore}/{match.scorecard.teamBWickets}
              </p>
              <p>Overs: {match.scorecard.teamAOvers} vs {match.scorecard.teamBOvers}</p>
            </div>
          )}
        </li>
      </Link>
      ...
      <div className="pt-24 w-full max-w-6xl mx-auto space-y-12">
        <AnimatePresence mode="popLayout">
          {tournaments.map(tournament => {
            ...
            const scheduledMatches = tournament.matches.filter(match => !match.isLive && !match.isCompleted);
